<script setup>
	...
	const fetchLessonDetails = async (lessonId) => {
		...
		try {
			...
			const theorySubjects = await axios.get(
				apiEndpoint + '/subjects/lesson/' + lesson + '/category/1'
			)
			const practiceSubjects = await axios.get(
				apiEndpoint + '/subjects/lesson/' + lesson + '/category/2'
			)
...
<template>
	<section class="summary">
		...
		<Survey
			title="Теория"
			type="theory"
			:questionsArray="theorySubjectsAndQuestions"
			v-auto-animate
		/>
		<Survey
			title="Практика"
			type="practice"
			:questionsArray="practiceSubjectsAndQuestions"
			v-auto-animate
		/>
	</section>
</template>
